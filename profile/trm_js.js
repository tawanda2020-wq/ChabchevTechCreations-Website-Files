...
function initializeCardAnimations() {
  ...
  document
    .querySelectorAll(
      ".skill-category, .portfolio-item, .service-card, .contact-card"
    )
    .forEach((card) => {
...
document.addEventListener("keydown", function (e) {
  ...
  if (e.key === "ArrowRight" || e.key === "ArrowDown") {
    ...
    const nextButton = document.querySelector(
      `[onclick="showSection('${sections[nextIndex]}')"]`
    );
    nextButton.click();
    ...
    const prevButton = document.querySelector(
      `[onclick="showSection('${sections[prevIndex]}')"]`
    );
    prevButton.click();
  }
});
    ...
    if (Math.abs(diffX) > Math.abs(diffY)) {
      if (Math.abs(diffX) > 50) {
        ...
        if (diffX > 0 && currentIndex < sections.length - 1) {
          ...
          const nextButton = document.querySelector(
            `[onclick="showSection('${sections[currentIndex + 1]}')"]`
          );
          nextButton.click();
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .querySelectorAll(".skill-category, .portfolio-item, .service-card")
    .forEach((el) => {
